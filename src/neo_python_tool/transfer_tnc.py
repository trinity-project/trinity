...
def main():
    ...
    # input_data=toArray(input)
    # output_data=toArray(output)
    # print (type(op_data))
    ...
    print ("rawtx:",rawtx_data)  #waiting for relay

    send_rawtransaction(rawtx_data)
...
if __name__ == "__main__":
    ...
    gas = get_gas_balance(address_from)
    print(gas)
    if gas < 0.001:
        print("no balance")
        sys.exit()
    gas_vout = get_gas_vouts(address_from)
    print(gas_vout)


    for item in gas_vout:
        if item["value"] >= 0.001:
            print (item)
            input_txid = item["txId"][2:]
            preIndex = item["voutNumber"]
            gas_change = item["value"] - 0.001

            print (input_txid,preIndex,gas_change)
            break
